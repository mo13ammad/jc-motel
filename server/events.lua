...
RegisterNetEvent('motels:server:toggleDoorlock', function(name, uniqueID, state)
    ...
    MySQL.query('SELECT owner FROM `jc_ownedmotels` WHERE `name` = ?', {name}, function(ownerResult)
        ...
        -- If not the owner, check if they are a valid renter
        MySQL.query('SELECT * FROM `jc_motels` WHERE `motel` = ? AND `uniqueid` = ?', {name, uniqueID}, function(rentalResult)
            if rentalResult and rentalResult[1] then
                local rental = rentalResult[1]
                if rental.renter == citizenid then
                    ...
                    -- Not the renter, deny access
                    QBCore.Functions.Notify(src, Lang['not_your_room'], 'error', 5000)
                end
            else
                -- No rental record, deny access
                QBCore.Functions.Notify(src, Lang['room_not_rented'], 'error', 5000)
            end
        end)
    end)
end)
